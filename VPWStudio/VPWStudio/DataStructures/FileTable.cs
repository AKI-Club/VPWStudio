...
namespace VPWStudio
{
	...
	public class FileTable : IXmlSerializable
	{
		...
		/// <param name="_romOffset">Location of first file in ROM.</param>
		public void WriteMidwaydec(StreamWriter sw, int _ftOffset, UInt32 _romOffset)
		{
			...
			{
				fte.Value.WriteMidwaydecEntry(sw, _romOffset);
			}

			// filetable entry
			sw.WriteLine(String.Format("# Add 0x{0:X} to each entry for actual offset.", _romOffset));
			sw.WriteLine(String.Format(
