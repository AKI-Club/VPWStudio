...
namespace VPWStudio
{
	public partial class SwapWrestler_Revenge : Form
	{
		/// <summary>
		/// List of stables
		/// </summary>
		...
		public SwapWrestler_Revenge(SortedList<int, StableDefinition> _stables, int _wres1Stable, int _wres1Index)
		{
			Stables = _stables;
			...
			labelSwapWres1.Text = String.Format(labelSwapWres1.Text, Stables[Wrestler1_CurStable].WrestlerPointers[Wrestler1_CurIndex], Wrestler1_CurStable);
		}
		...
		private void PopulateWrestlers()
		{
			...
			for (int i = 0; i < FreeStables[stableNum].WrestlerPointers.Length; i++)
			{
				if (FreeStables[stableNum].WrestlerPointers[i] != 0)
				{
					lbDestStableWrestlers.Items.Add(String.Format("{0:X8}", FreeStables[stableNum].WrestlerPointers[i]));
				}
			}
